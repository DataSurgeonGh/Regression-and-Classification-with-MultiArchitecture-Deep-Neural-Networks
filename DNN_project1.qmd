...
options(warn = -1)
keras3::keras$utils$disable_interactive_logging()
```
...
```{r}
ggplot(df_hourly[1:10, ], aes(x = date_time, y = t_deg_c)) +
  geom_line() +
  labs(
    title = "Hourly Temperature (first 10 points)",
    x = "Time",
    y = "Temperature"
  )
```
...
ggplot(df_jena, aes(x = date_time, y = t_deg_c)) +
  geom_line() +
  labs(
    title = "Full Temperature Time Series",
    x = "Time",
    y = "Temperature (°C)"
  )
```
...
ggplot(df_10_days, aes(x = date_time, y = t_deg_c)) +
  geom_line() +
  labs(
    title = "First 10 Days (Hourly Temperature)",
    x = "Date / Time",
    y = "Temperature (°C)"
  )
```
...
The correlation is high among these variables because, it appears temperature related variables are highly correlated. The negative values are those that are "inversely related"
...
ggplot(df_long, aes(x = variable, y = value)) +
  ...
  labs(
    title = "Parallel Boxplots of Predictors",
    x = "", y = ""
  ) +
...
###############
###############
#redundant generator --- to be removed
...
histories$base <- model_base %>% fit(
  train_gen_reg,
  steps_per_epoch = 100, #originally set at 200
  epochs = 20, #originally set at 30
  validation_data = val_gen_reg,
  validation_steps = 50
 )
```
...
histories$dropout <- model_drop %>% fit(
  train_gen_reg,
  steps_per_epoch = 100, # originally set at 200
  epochs = 20,  #originally set at 20
  validation_data = val_gen_reg,
  validation_steps = 50
)
```
...
histories$l2 <- model_l2 %>% fit(
  train_gen_reg,
  steps_per_epoch = 20,  #originally set at 200
  epochs = 10,  # originally set at 20
  validation_data = val_gen_reg,
  validation_steps = 50
)
```
...
history <- model %>% fit(
  x_train, y_train,
  epochs = 20,  #originally set at 30
  batch_size = 60, #originally set at 128
...
#)
model_dnn_jena = model
# Option 1 — Summary table (most reliable, always works)
model_dnn = model
summary(model_dnn_jena)
summary(model)
```

```{r}
#The number of trainable parameters can be found in the model summary
keras3::count_params(model_dnn_jena)
```
...
hist_base <- model_base %>% fit(
  x_train, y_train,
  epochs = 30,  #originally at 30
  batch_size = 128,  # originally at 128
  validation_split = 0.2,
  callbacks = list(early_stop),
  verbose = 0
)
...
```{r}
# predict — returns probabilities, shape (10000, 10)
preds_prob <- model %>% predict(x_test)

# convert one-hot y_test back to class labels
y_true <- apply(y_test, 1, which.max) - 1L   # 0–9

# convert predicted probabilities to class labels
y_pred <- apply(preds_prob, 1, which.max) - 1L   # 0–9
...
```{r}
#parameter count
cnn_params <- sum(sapply(model_cnn$trainable_weights, function(w) {
  shape <- as.integer(w$shape)
  prod(shape)
}))
...
```{r}
dnn_params <- sum(sapply(model_dnn$trainable_weights, function(w) {
  shape <- as.integer(w$shape)
  prod(shape)
}))
dnn_params
...
```{r}
# flatten data
x_train_flat <- array_reshape(x_train, c(nrow(x_train), 28 * 28))
x_test_flat  <- array_reshape(x_test,  c(nrow(x_test),  28 * 28))
...
```{r}
# accuracy
eval <- model_dnn %>% evaluate(x_test_flat, y_test, verbose = 0)
dnn_test_acc <- eval["accuracy"]
```
...
However After running it again, the accuracy appears to be the same\
...
history <- model_cnn1d %>% fit(
  train_gen_reg,
  steps_per_epoch = 20,   # originally 200
  epochs = 30,
  validation_data = val_gen_reg,
  validation_steps = 50,
  callbacks = list(early_stop)
...
history_rnn <- model_rnn %>% fit(
  train_gen_reg,
  steps_per_epoch = 50, #originly set at 200
  epochs = 30,
  validation_data = val_gen_reg,
  validation_steps = 50,
  callbacks = list(early_stop),
...
data <- df_jena %>%
  select(`t_deg_c`,`p_mbar`,`rh_percent`, `v_pmax_mbar`, 
         `h2oc_mmol_mol`, `wd_deg`, `wv_m_s`) %>%
  as.matrix()

# Normalization
mean_vals <- colMeans(data)
std_vals  <- apply(data, 2, sd)
data_norm <- scale(data, center = mean_vals, scale = std_vals)

cat("Data shape:", dim(data_norm), "\n")

# --------------------- 2. Data Generator ---------------------
create_generator <- function(data, lookback, delay, min_index, max_index,
                             batch_size = 128, shuffle = FALSE) {
  
  function() {
    if (shuffle) {
      rows <- sample(min_index:max_index, size = batch_size, replace = FALSE)
    } else {
      if (!exists("start_idx", envir = .GlobalEnv)) start_idx <<- min_index
      rows <- seq(start_idx, min(start_idx + batch_size - 1, max_index))
      start_idx <<- start_idx + batch_size
      if (start_idx > max_index) start_idx <<- min_index
    }
    
    samples <- array(0, dim = c(length(rows), lookback, ncol(data)))
    targets <- matrix(0, nrow = length(rows), ncol = 4)
    
    for (j in seq_along(rows)) {
      indices <- (rows[j] - lookback):(rows[j] - 1)
      samples[j, , ] <- data[indices, ]
      
      next_temp <- data[rows[j] + delay, 1]
      
      if (next_temp < 0)          cls <- 0
      else if (next_temp < 10)    cls <- 1
      else if (next_temp < 20)    cls <- 2
      else                        cls <- 3
...
n <- nrow(data_norm)
train_max <- as.integer(n * 0.5)
val_max   <- as.integer(n * 0.75)

train_gen <- create_generator(data_norm, lookback, delay, 
                              lookback + 1, train_max, batch_size, TRUE)

val_gen   <- create_generator(data_norm, lookback, delay, 
                              train_max + 1, val_max, batch_size, FALSE)

test_gen  <- create_generator(data_norm, lookback, delay, 
                              val_max + 1, n - delay - 1, batch_size, FALSE)
...
n_features <- ncol(data_norm)
model <- build_lstm_cls(lookback, n_features)
...
probs <- predict(model, test_gen, steps = steps)
preds <- apply(probs, 1, which.max) - 1   # 0,1,2,3
...
for (i in 1:steps) {
  batch <- test_gen()
  y_batch <- batch[[2]]
  true_cls <- apply(y_batch, 1, which.max) - 1
  actuals <- c(actuals, true_cls)
}

# --------------------- 7. Fixed Metrics ---------------------
accuracy <- mean(preds == actuals)

# Robust Confusion Matrix (force all 4 classes)
cm_table <- table(factor(preds, levels = 0:3), 
                  factor(actuals, levels = 0:3))
...
# Manual Macro-F1 (safe version)
macro_f1 <- 0
for (k in 0:3) {
  tp <- cm_table[as.character(k), as.character(k)]
  fp <- sum(cm_table[as.character(k), ]) - tp
  fn <- sum(cm_table[, as.character(k)]) - tp
  ...
  cat(sprintf("Class %d - Precision: %.4f, Recall: %.4f, F1: %.4f\n", 
...
class_labels <- c("Very Cold (<0°C)", "Cold (0-10°C)", "Mild (10-20°C)", "Warm (>20°C)")
cm_df$Predicted <- factor(cm_df$Predicted, levels = 0:3, labels = class_labels)
cm_df$Actual    <- factor(cm_df$Actual,    levels = 0:3, labels = class_labels)

ggplot(cm_df, aes(x = Actual, y = Predicted, fill = Count)) +
  geom_tile(color = "white") +
  geom_text(aes(label = Count), color = "black", size = 4) +
  scale_fill_gradient(low = "white", high = "steelblue") +
  labs(title = "Confusion Matrix - LSTM 4-Class Temperature Classification",
       subtitle = paste("Test Accuracy:", round(accuracy, 4), "| Macro-F1:", round(macro_f1/4, 4)),
       x = "Actual Class",
       y = "Predicted Class") +
  theme_minimal() +
...
# --------------------- 4. Test Evaluation (Fixed for keras3) ---------------------
eval <- model_lstm %>% evaluate(x_test_seq, y_test, verbose = 0)

# Safe extraction for keras3
...
probs <- model_lstm %>% predict(x_test_seq)
preds <- apply(probs, 1, which.max) - 1

# Accuracy check
acc_check <- mean(preds == y_test_labels)
cat("Confirmed Accuracy:", round(acc_check, 4), "\n")

# Confusion Matrix
cm_table <- table(Predicted = factor(preds, levels = 0:9),
                  Actual    = factor(y_test_labels, levels = 0:9))
...
ggplot(cm_df, aes(x = Actual, y = Predicted, fill = Count)) +
  ...
  scale_fill_gradient(low = "white", high = "steelblue") +
  labs(title = "Confusion Matrix - LSTM on MNIST",
       subtitle = paste("Test Accuracy:", round(test_acc, 4)),
       x = "Actual Digit", 
       y = "Predicted Digit") +
  theme_minimal() +
...
LSTM’s inductive bias: LSTMs assume sequential/temporal dependencies. Treating image rows as a time series forces the model to learn spatial relationships indirectly through recurrence. While LSTM can still capture vertical dependencies reasonably well, it lacks the explicit 2D spatial priors that convolutions provide. Although the LSTM is a powerful sequential model, it imposes a **temporal inductive bias**: Thereby
...
CNN’s inductive bias: CNNs have built-in assumptions that are highly suitable for images locality (nearby pixels matter more), translation invariance (via pooling), and hierarchical feature learning (edges → textures → objects). This bias allows CNNs to learn very efficiently from spatial structure
